...
from ..impls.overlap import a2a_ffn_overlap_forward, a2a_ffn_overlap_balance
from . import losses


class pMOELayer(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if experts_type == 'custom':
            ...
            self.experts = fused_experts.BalanceModule(**experts)

        self.experts.update(self)
        self.hidden_dim = experts["hidden_size_per_expert"]
...
#             y = C.all_to_all(y, 0, 1, use_2dh=self.use_2dh, group=self.group)
        ...
        def expert_fn(input):
            # return self.expert_local(expert_input, original_shape[-reserve_dims:])
            return self.experts(input, self)
        y = a2a_ffn_overlap_forward(input, self.gate, self.model_dim, self.hidden_dim, expert_fn=expert_fn, a2a_ffn_overlap_degree=a2a_ffn_overlap_degree,
                                    use_2dh=self.use_2dh, group=self.group, compress_name=self.compress_name,
                                    comm_name=self.comm_name)
