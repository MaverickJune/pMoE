...
# Generate unique nsys report filenames
nsys_output="$output_dir/profile_pmoe_fmoe_node${node_rank}.nsys-rep"

# Run Nsight Systems profiling with unique filenames
env NCCL_SOCKET_IFNAME=ens4f0np0 nsys profile --output=$nsys_output \
  --trace=cuda,nvtx \
