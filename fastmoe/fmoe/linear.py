...
class pMoELinear(nn.Module):
    ...
    def __init__(
        ...
        self.in_feat = in_feat # original feature
        self.out_feat = out_feat # original feature
        self.comm = ctx.mesh.get_group("tp")
        self.rank = 0

        if self.comm is not None:
            size = dist.get_world_size(self.comm)
            self._in_feat = in_feat // size # pMoE feature
            self.weight = nn.Parameter(torch.Tensor(self.total_experts, self.out_feat, self._in_feat))
        else: # deprecated
            raise ValueError("tensor parallel is None. Stopping the process.")
        if bias:
            self.bias = nn.Parameter(torch.zeros(self.total_experts, out_feat))
        else:
            self.register_parameter("bias", None)
    ...
    def extra_repr(self) -> str:
        return "total_experts={}, in_features={}, \
        out_features={}, bias={}, rank={}".format(
            self.total_experts,
            self.in_feat,
            self.out_feat,
