...
void fmoe_cuda_linear_forward_impl(
    ...
    fprintf(stderr, "[DEBUG Rank %ld]: Entering Linear Forward\n", smgr->device);
    // std::lock_guard<std::mutex> lock(smgr_mutex);
    for (int i = 0, ptr = 0; i < num_expert; ++i) {
        fprintf(stderr, "[DEBUG Rank %ld]:, num_experts = %zu\n",smgr->device, i);
        smgr->sync(i);
        fprintf(stderr, "[DEBUG Rank %ld]:, Sync Test\n",smgr->device);
        if (expert_count[i] == 0) {
            continue;
        }
        // std::lock_guard<std::mutex> lock(smgr_mutex);
    ...
    fprintf(stderr, "[DEBUG Rank %ld]: End Processing Linear ",smgr->device);
}
template <typename scalar_t>
