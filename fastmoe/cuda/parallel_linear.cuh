...
void fmoe_cuda_linear_forward_impl(
    ...
    for (int i = 0, ptr = 0; i < num_expert; ++i) {
        ...
        checkCudaErrors(cublasXgemm(
                ...
                CUBLAS_OP_T,
                CUBLAS_OP_N,
                out_feat, expert_count[i], in_feat, // m, n, k
                &alpha,
                weight + i * in_feat * out_feat, in_feat, // m x k -> k x m
                input_buf + ptr * in_feat, in_feat, // k x n
                &beta,
                output_buf + out_feat * ptr, out_feat // m,n
                ));
