...
from fastmoe.fmoe.transformer import pMoETransformerMLP, FMoETransformerMLP
import torch
import glog
...
def get_model_from_hf(model_name="meta-llama/Llama-3.1-70B-Instruct", partial=0.4, gpu_idx=-1, model_dict=None, enable_cache=False, pad_token_id=-1):
...
def model_wrapper_fmoe(model, system_config=None, args=None):
    ...
    for i in range(len(model.model.layers)):
        model.model.layers[i].mlp = FMoETransformerMLP(num_expert=total_experts, d_model=d_model, d_hidden=d_hidden, top_k=top_k, 
                                                       world_size=world_size, moe_group=moe_group, gate=gate, is_llama=True, imbalance_level=args.imbalance_level, max_idx=args.max_idx).to(torch.bfloat16).to(gpu_idx)
